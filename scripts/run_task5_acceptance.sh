...
if [[ ! -x "${python_bin}" || ! -f "${isolated_launcher}" \
  || -L "${isolated_launcher}" || ! -f "${verifier}" || -L "${verifier}" ]]; then
  echo "FAIL: Task 5 registered Python execution envelope is unavailable" >&2
  exit 1
fi
...
final_head="$(/usr/bin/git rev-parse HEAD)"
final_parent_record="$(
  /usr/bin/git rev-list --parents -n 1 "${final_head}"
)"
if [[ "$(/usr/bin/git branch --show-current)" != "main" \
  || "$(/usr/bin/git log -1 --pretty=%s)" \
    != "feat(evaluation): unify proof manifests and baseline feasibility" \
  || "${final_parent_record}" != "${final_head} ${task4_commit}" \
  || -n "$(/usr/bin/git status --porcelain)" \
  || "$(/usr/bin/git remote get-url origin)" \
    != "git@github.com:Cunyu-Liu/d2t-rna.git" ]]; then
  echo "FAIL: final Task 5 runner requires the clean registered commit" >&2
  exit 1
fi
if [[ -z "${TASK5_FINAL_RUN_ID:-}" \
  || ! "${TASK5_FINAL_RUN_ID}" =~ ^task5-final-[0-9]{8}T[0-9]{6}\+0800$ ]]; then
  echo "FAIL: TASK5_FINAL_RUN_ID is not canonical" >&2
  exit 1
fi

run_dir="${artifact_root}/runs/${TASK5_FINAL_RUN_ID}"
pycache_dir="${run_dir}/pycache"
junit_dir="${run_dir}/junit"
fixture_dir="${run_dir}/fixture"
snapshot_dir="${run_dir}/snapshots"
if [[ ! -d "${run_dir}" || -L "${run_dir}" ]]; then
  echo "FAIL: final Task 5 run directory is unavailable" >&2
  exit 1
fi
for output in "${pycache_dir}" "${junit_dir}" "${fixture_dir}" "${snapshot_dir}"; do
  if [[ -e "${output}" || -L "${output}" ]]; then
    echo "FAIL: final Task 5 output already exists: ${output}" >&2
    exit 1
  fi
done
for forbidden_path in \
  conftest.py sitecustomize.py usercustomize.py pytest.ini tox.ini setup.cfg; do
...
if [[ -n "${execution_symlink}" || -n "${legacy_bytecode}" \
  || -n "${native_extension}" ]]; then
  echo "FAIL: unsafe Task 5 execution-tree input detected" >&2
  exit 1
fi
...
printf "TASK5_FINAL_RUNNER_SCHEMA=d2t_rna.task5_final_runner.v1\n"
printf "TASK5_FINAL_RUN_ID=%s\n" "${TASK5_FINAL_RUN_ID}"
...
if [[ "${post_test_source_index_sha}" != "${pre_test_source_index_sha}" ]]; then
  echo "FAIL: Task 5 source index changed during final run" >&2
  exit 1
fi
...
task5_python "${verifier}"
printf "TASK5_MANIFEST_VERIFIED\n"
printf "TASK5_LIVE_MANIFEST_REPLAY_PASS\n"
...
while IFS= read -r -d '' path; do
  if [[ -f "${path}" ]] && grep -IEn -- "${secret_pattern}" "${path}"; then
    secret_findings=1
  else
    grep_status=$?
    if [[ -f "${path}" ]] && (( grep_status != 1 )); then
...
while IFS= read -r -d '' path; do
  if [[ -f "${path}" ]] && (( $(wc -c < "${path}") > 5242880 )); then
